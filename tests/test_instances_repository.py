...
# ---------------------------------------------------------------------------
# WU-2: hook_token — RED tests
# ---------------------------------------------------------------------------


def test_create_generates_hook_token(repo: InstancesRepository, proj_id: str) -> None:
    """InstancesRepository.create() must set a non-empty hook_token."""
    inst = repo.create(project_id=proj_id, tmux_session_name="claude-remote-proj-tk000001")
    assert inst.hook_token is not None
    assert len(inst.hook_token) > 0


def test_create_two_instances_produce_distinct_tokens(
    repo: InstancesRepository, proj_id: str
) -> None:
    """Two consecutive creates must produce different hook_token values."""
    inst_a = repo.create(project_id=proj_id, tmux_session_name="claude-remote-proj-tk000002")
    inst_b = repo.create(project_id=proj_id, tmux_session_name="claude-remote-proj-tk000003")
    assert inst_a.hook_token != inst_b.hook_token, "hook_token values must be unique"


def test_get_returns_hook_token_field(repo: InstancesRepository, proj_id: str) -> None:
    """get() must map the hook_token column from the DB row."""
    inst = repo.create(project_id=proj_id, tmux_session_name="claude-remote-proj-tk000004")
    fetched = repo.get(inst.id)
    assert fetched is not None
    assert fetched.hook_token == inst.hook_token


def test_get_by_hook_token_returns_correct_instance(
    repo: InstancesRepository, proj_id: str
) -> None:
    """get_by_hook_token must return the matching instance."""
    inst = repo.create(project_id=proj_id, tmux_session_name="claude-remote-proj-tk000005")
    found = repo.get_by_hook_token(inst.hook_token)
    assert found is not None
    assert found.id == inst.id


def test_get_by_hook_token_returns_none_for_unknown(repo: InstancesRepository) -> None:
    """get_by_hook_token with an unknown token must return None."""
    assert repo.get_by_hook_token("totally-bogus-token") is None


def test_hook_token_is_url_safe_string(repo: InstancesRepository, proj_id: str) -> None:
    """hook_token generated by secrets.token_urlsafe(32) must be ~43 chars of URL-safe base64."""
    inst = repo.create(project_id=proj_id, tmux_session_name="claude-remote-proj-tk000006")
    # secrets.token_urlsafe(32) produces a string of ~43 chars (base64url, no padding)
    token = inst.hook_token
    assert len(token) >= 40, f"Token too short: {token!r}"
    # Only URL-safe characters (base64url alphabet + no padding)
    import re
